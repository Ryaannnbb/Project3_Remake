                    ...
                    <div class="card-header border-bottom pb-0">
                        <div class="d-sm-flex align-items-center">
                            <div>
                                ...
                                <p class="text-sm">See information about all order history</p>
                            </div>
                                    ...
                                    @if ($histories->count() > 0)
                                        @foreach ($histories as $pesanan)
                                            <tr>
                                                ...
                                                <td class="align-middle text-center text-sm">
                                                    <span
                                                        class="text-secondary text-xs font-weight-bold">{{ $pesanan->user->name }}</span>
                                                </td>
                                                <td class="align-middle text-center text-sm">
                                                ...
                                                <td class="align-middle text-center text-sm">
                                                    @if ($pesanan->status == 'pending')
                                                        <span
                                                            class="badge bg-warning text-dark">{{ ucfirst($pesanan->status) }}</span>
                                                    ...
                                                    @elseif ($pesanan->status == 'paid')
                                                        <span
                                                            class="badge bg-success text-dark">{{ ucfirst($pesanan->status) }}</span>
                                                ...
                                                <td class="align-middle text-center text-sm">
                                                    ...
                                                    @if ($pesanan->status == 'pending')
                                                        <form action="{{ route('pesanan.terima', $pesanan->id) }}"
                                                            ...
                                                            @csrf
                                                            <button type="button"
                                                                class="btn btn-success btn-acc mb-n1 mt-n1 p-2"
                                                        ...
                                                        <div class="modal fade" id="exampleModal{{ $pesanan->id }}"
                                                            ...
                                                            <div class="modal-dialog modal-dialog-centered">
                                                                <form class="modal-content"
                                                                    action="{{ route('pesanan.tolak', $pesanan->id) }}"
                                                                    method="POST">
                                                                    @csrf
                                                        ...
                                                        @enderror
                                                    @elseif ($pesanan->status == 'paid')
                                                        <a href="{{ route('pengiriman.create', $pesanan->id) }}"
                                                            class="btn btn-secondary mb-n1 mt-n1 p-2">Atur Pengiriman</a>
                                                    @endif
                                                </td>
                                            </tr>
                                        @endforeach
